...
#[derive(Accounts)]
#[instruction(public_inputs: [u8;96])]
pub struct WithdrawVariable<'info> {
    ...
    #[account(
        init_if_needed, 
        payer = user,
        seeds = [b"nullifier", public_inputs[32..64].as_ref()], 
        bump, 
        space = 0
      )]
...
pub struct MerkleMountainRange{
    ...
    pub whole_tree_root: [u8; 32],
    pub batch_number: u64,
    pub peaks: [[u8; 32]; 32], //Peaks to build merkle tree without storing everything
    pub depth: [u8; 32], //With each peak with associate a depth
    pub number_of_peaks: u8, //Max number of peaks is 255
    pub max_leaves: u64, //MAX number of leaves in a pool    
}

impl MerkleMountainRange {
    pub const MAX_SIZE: usize =  32 + 512 + 16 + 8 + 32  + 8 + 32 + 8 + 1024 + 32 + 1 + 8 ;
